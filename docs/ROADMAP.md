...
## Deferred Backlog · Transport Tracking workflow (deferred 2026-06-29)
- **Operational Exceptions column redesign** — replace the removed generic status badges with an operational-first "needs attention" column that **names the action** (e.g. "Poids fournisseur manquant"), never a generic status, and never duplicates the gap column / filters / banner. Per [operational-fields-only rule](#) — design before building.
- **Transport Tracking workflow improvements (Phase 5.3B)** — entry workflow: auto Net = Brut − Tare, default dates, Save & New, duplicate previous ticket, remember last values. Paused (SharePoint production bug has priority).
- **Operational visibility refinements** — further surfacing of exceptions where operators work, without duplicate representation.
