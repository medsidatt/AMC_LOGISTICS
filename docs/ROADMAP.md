...
| **Inspections SPA workspace** (Fleet — completes Fleet product) — single `/hse/inspections` page orchestrating both controllers (`HseController` read/sign + `LogisticsInspectionController` authoring); `InspectionFormDrawer` (create+edit, fetches refs as JSON, reuses CameraCapture + data-driven checklist) replaces Create/Edit pages; `InspectionDetailsDrawer` (full checklist + issues + photo + in-drawer Sign + PDF) replaces Show page + signature modal; `DataTable` replaces raw table; removed the stale embedded Maintenance section (duplicate of `/maintenance`); 4 pages → 1 workspace; 0 full-page CRUD nav; no products/no delete (compliance records) | ✅ | `feature/spa-maintenance-workspace` |
| **Roles SPA workspace** (Administration — Phase 4.1, **reference standard**) — single `/roles` workspace; `RoleFormDrawer` (create+edit, `PermissionMatrix` reused UNCHANGED) + `RoleDetailsDrawer` (`DetailPanel` + grouped permissions) replace Index+Create+Edit+Show; **URL-driven drawer state** (`?create=1`/`?view={id}`/`?edit={id}`, History-API only, popstate Back/Forward, one drawer at a time, all from index payload — no per-drawer fetch); `ActionButtons` callbacks replace href nav; removed 7 SPA violations (`window.location`×2, `history.back`×5), dead `create`/`edit`/`show` routes, dead `getData()`, dead `resources/views/pages/roles/` Blade dir; standard documented in [docs/workspace-standard.md](workspace-standard.md) (now the **platform-wide** reference) | ✅ | `feature/spa-maintenance-workspace` |
| **`useWorkspaceDrawer` hook** (shared foundation) — generic URL-driven drawer state machine (parse/sync URL, create/view/edit modes, popstate Back/Forward, refresh restore, single-drawer guarantee, preserves `?page`/`?search`); zero business logic; mandatory for future workspaces. Roles retrofitted onto it (reference consumes the shared foundation) | ✅ | `feature/spa-maintenance-workspace` |
| **Users SPA workspace** (Administration — Phase 4.2, standardization only) — Modals → Drawers via the shared hook; `UserFormDrawer` (create+edit, `PermissionMatrix` overlay + role checkboxes reused UNCHANGED) + `UserDetailsDrawer` (`DetailPanel`); `PageHeader`/`FormActions`; browser `confirm()` (suspend) → shared `ConfirmDialog`; **zero backend/route/validation changes** (UserController, Store/UpdateUserRequest, invitation-on-create, Microsoft SSO, password mgmt, guard logic all untouched) | ✅ | `feature/spa-maintenance-workspace` |
