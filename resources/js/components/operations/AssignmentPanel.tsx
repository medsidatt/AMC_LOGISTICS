...
/** Data the crew roster needs — produced by DispatchWorkspaceService::crewData. */
...
/**
 * Driver↔truck crew roster — presentational. Shared by the standalone
 * /logistics/affectations page and the Operations Dispatch tab (crew sub-tab).
 * Writes via the single assignment save path (TruckDriverAssignmentService) and
 * use back(), so they return to whichever surface posted.
 */
export default function AssignmentPanel({ trucks, availableDrivers, history, roles }: AssignmentPanelData) {
    ...
    const SlotCell = ({ truck, role, slot }: { truck: TruckRow; role: string; slot: Slot | null }) => {
        if (!slot) {
            return (
                <button
                    type="button"
            ...
            <div className="flex items-start justify-between gap-3 flex-wrap">
                <div>
                    <div className="flex items-center gap-2">
                        ...
                        <h2 className="text-lg font-semibold">Affectations chauffeurs — camions</h2>
                    </div>
                        ...
                        <tbody className="divide-y divide-[var(--color-border)]">
                            {trucks.map((t) => (
                                <tr key={t.id} className={clsx('align-top', t.parking && 'bg-amber-50/50 dark:bg-amber-900/10')}>
                                    ...
                                    <td className="px-4 py-3"><SlotCell truck={t} role="titulaire" slot={t.titulaire} /></td>
                                    <td className="px-4 py-3"><SlotCell truck={t} role="assistant" slot={t.assistant} /></td>
                                </tr>
                            ))}
                        </tbody>
                    </table>
