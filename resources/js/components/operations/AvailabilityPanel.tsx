...
import { ChevronLeft, ChevronRight, Plus, Trash2, Gauge } from 'lucide-react';
...
export interface AvailabilityPanelData {
...
interface Props extends AvailabilityPanelData {
    /** Month navigation — host decides where it goes (standalone vs Planning tab). */
    onChangeMonth: (anchorIso: string) => void;
}
...
/**
 * Fleet availability — presentational. Shared by the standalone /logistics/availability
 * page and the Planning workspace section. Window add/remove post via the single
 * availability save path (back()); month nav is delegated via onChangeMonth.
 */
export default function AvailabilityPanel({ period, fleet, trucks, windows, truckOptions, types, onChangeMonth }: Props) {
    ...
    return (
        <div className="space-y-5">
            <div className="flex items-center justify-end gap-2">
                <button type="button" onClick={() => onChangeMonth(shiftMonth(period.anchor, -1))} aria-label="Mois précédent" className="p-2 rounded-lg hover:bg-[var(--color-surface-hover)] cursor-pointer"><ChevronLeft size={18} /></button>
                <span className="text-sm font-medium capitalize min-w-[9rem] text-center">{period.label}</span>
                <button type="button" onClick={() => onChangeMonth(shiftMonth(period.anchor, 1))} aria-label="Mois suivant" className="p-2 rounded-lg hover:bg-[var(--color-surface-hover)] cursor-pointer"><ChevronRight size={18} /></button>
            </div>
